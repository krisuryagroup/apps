...
export class BottomNavComponent implements OnInit, OnDestroy {
  ...
  navItems = [
    { label: UI_TEXT.HOME, icon: 'home', route: '/home', key: 'home', altRoutes: ['/'] },
    { label: 'Search', icon: 'search', route: '/listing', key: 'search', altRoutes: ['/search'] },
    { label: UI_TEXT.FAVORITES, icon: 'favorite', route: '/favorites', key: 'favorites', altRoutes: [] },
    { label: 'Account', icon: 'person', route: '/account', key: 'account', altRoutes: [] }
  ];
  ...
  private setActiveFromRoute(url: string) {
    ...
    for (const item of this.navItems) {
      if (url === item.route || 
          ...
          item.altRoutes?.some(altRoute => url === altRoute || url.startsWith(altRoute + '/'))) {
