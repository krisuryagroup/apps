...
export class BannerComponent implements OnInit, OnDestroy {
  ...
  @Input() scrollInterval: number = 4000; // 4 seconds
  ...
  private isLoadingBanners: boolean = false; // Prevent multiple simultaneous loads
  ...
  onTouchEnd(event: TouchEvent) {
    ...
    if (Math.abs(deltaX) > Math.abs(deltaY) && Math.abs(deltaX) >= this.SWIPE_MIN_DIST) {
      if (deltaX < 0) {
        this.nextBanner();
      } else {
        this.prevBanner();
    ...
    try {
      ...
      if (this.banners.length === 0) {
        ...
        if (!banner.versionCondition || !banner.versionTarget) return true;
          const cmp = compareApplicationVersions(appVersion, banner.versionTarget);
        ...
        this.bannerService.setActiveBannerConfigs(this.banners[this.currentBanner]?.configs);
      }
