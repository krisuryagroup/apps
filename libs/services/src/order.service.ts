...
export class OrderService {
    ...
    try {
      ...
      const orderToSave = {
        ...
        scheduledPickupTime: order.scheduledPickupTime
          ? Timestamp.fromDate(order.scheduledPickupTime)
          : null,
        items: order.items,
        ...
        statusTimeline:
          order.statusTimeline?.map((timeline) => ({
      ...
      console.log(
        `Order created successfully with document ID: ${order.orderId}`,
      );
    ...
    try {
      ...
      querySnapshot.forEach((doc) => {
        ...
        const order: Order = {
          ...
          estimatedDeliveryTime: docData['estimatedDeliveryTime']
            ? convertTimestamp(docData['estimatedDeliveryTime'])
            : undefined,
          statusTimeline:
            docData['statusTimeline']?.map((timeline: any) => ({
              status: timeline.status,
  ...
  getTimelineForStatus(order: OrderDisplay, status: string): Date | null {
    ...
    const timelineEntry = order.statusTimeline.find(
  ...
  private setCachedOrderHistory(userId: string, orders: OrderDisplay[]): void {
    try {
      ...
      console.log(
        `💾 Order history cached successfully (${Math.round(duration / 1000 / 60 / 60 / 24)} days)`,
      );
    } catch (error) {
