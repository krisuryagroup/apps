...
export class PricingService {
  ...
  async loadPricingConfig(): Promise<PricingConfig> {
    ...
    try {
      ...
      if (pricingSnap.exists()) {
        ...
        console.warn(
          '⚠️ PricingService: Configuration not found in Firebase, using defaults',
        );
  ...
  async calculatePricing(
    ...
    const packagingCharge = this.calculatePackagingCharge(
      input.orderType,
      config,
    );
  ...
  private calculateDeliveryCharge(
    ...
    const isEligibleForFreeDelivery =
      subtotal >= config.delivery.free_delivery_above;
  ...
  private calculateSavings(
    ...
    const totalSavings =
      freeDelivery + freePackaging + freePlatformFee + freeGst + couponDiscount;
  ...
  private determineVisibility(
    ...
    return {
      ...
      showPackaging:
        charges.packaging.isVisible &&
        (charges.packaging.calculated > 0 || charges.packaging.applied > 0),
      showPlatformFee:
        charges.platformFee.isVisible &&
        (charges.platformFee.calculated > 0 || charges.platformFee.applied > 0),
      showGst:
        charges.gst.isVisible &&
        (charges.gst.calculated > 0 || charges.gst.applied > 0),
