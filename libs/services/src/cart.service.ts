...
export class CartService {
  ...
  private loadCartFromStorage() {
    try {
      const storedCart = this.cacheService.getCachedData<CartItem[]>(this.CART_STORAGE_KEY);
      if (storedCart) {
        this.items = storedCart;
        console.log('🛒 CartService: Loaded cart for restaurant:', this.cacheService.getCurrentRestaurantId(), 'Items:', this.items.length);
      } else {
        this.items = [];
      }
    } catch (error) {
  ...
  private saveCartToStorage() {
    try {
      ...
      console.log('💾 CartService: Saved cart for restaurant:', this.cacheService.getCurrentRestaurantId(), 'Items:', this.items.length);
    } catch (error) {
  ...
  private getEffectivePrice(item: Product): number {
    if (item.hasVariations && item.selectedVariationId && item.variations) {
      const selectedVariation = item.variations.find(v => v.id === item.selectedVariationId);
  ...
  async removeFromCart(item: Product, isRequestFromCartPage: boolean = false): Promise<boolean> {
    ...
    if (!isRequestFromCartPage && item.hasVariations && item.variations) {
      ...
      const cartItemsWithVariations = this.items.filter(ci => 
        ci.name === item.name && ci.selectedVariationId
      );
  ...
  async refreshCartItemsFromFirebase(): Promise<void> {
    ...
    try {
      ...
      if (productIds.length === 0) {
        console.warn('⚠️ Cart items do not have IDs, cannot refresh from Firebase');
        return;
      }

      // Fetch fresh product data from Firebase
      const freshProducts = await this.productsService.getProductsByIds(productIds);
      ...
      this.items = this.items.map(cartItem => {
        ...
        if (freshProduct) {
          ...
          const offerStatusChanged = cartItem.isOfferDisabled !== freshProduct.isOfferDisabled;
