...
export class AddressApiService {
  ...
  getAddresses(): Observable<Address[]> {
    ...
    return this.http
      .get<AddressDto[]>(`${this.baseUrl}${CustomerEndpoints.addresses.list()}`)
      .pipe(
        map((dtos) => dtos.map((dto) => UserMapper.toAddress(dto))),
        tap((addresses) =>
          this.cache.set(ADDRESSES_KEY, addresses, { ttlHours: 1 / 12 }),
        ),
      );
  }
  ...
  createAddress(address: Omit<Address, 'id'>): Observable<Address> {
    ...
    return this.http
      .post<AddressDto>(
        `${this.baseUrl}${CustomerEndpoints.addresses.list()}`,
        request,
      )
      .pipe(
  ...
  updateAddress(
    ...
    return this.http
      .put<AddressDto>(
        `${this.baseUrl}${CustomerEndpoints.addresses.byId(id)}`,
        request,
      )
      .pipe(
        map((dto) => UserMapper.toAddress(dto)),
  ...
  deleteAddress(id: string): Observable<void> {
    return this.http
      .delete<void>(`${this.baseUrl}${CustomerEndpoints.addresses.byId(id)}`)
      .pipe(tap(() => this.cache.invalidate(ADDRESSES_KEY)));
  }
}
