...
import type { GeoNearbyPlace, GeoSearchSuggestion } from '../google-geocoding.service';
...
export class GeocodingApiService {
  ...
  searchAddresses(query: string, nearLat: number, nearLng: number): Observable<GeoSearchSuggestion[]> {
    return from(this.geocoding.searchAddresses(query, { lat: nearLat, lng: nearLng }));
  }
  ...
  getNearbyBusinesses(lat: number, lng: number, radiusKm = 10): Observable<NearbyBusiness[]> {
    return this.http.get<NearbyBusiness[]>(`${this.baseUrl}/api/businesses/nearby`, {
