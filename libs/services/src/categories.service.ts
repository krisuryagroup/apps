  ...
  async getCategories(): Promise<Category[]> {
    ...
    try {
      ...
      const filteredCategories = categories.filter(
        (cat) => cat.status === true && cat.isEnabledForOnlineOrders === true,
      );
  ...
  private getCachedCategories(): Category[] | null {
    try {
      ...
      const categories = cached.map((cat: Category) => ({
        ...cat,
        imageURL: cat.imageURL.startsWith('gs://')
          ? FirebaseStorageUtil.convertStorageUrlToHttps(cat.imageURL)
          : cat.imageURL,
      }));

      console.log(
        '🚀 CategoriesService: Using cached categories for restaurant:',
  ...
  private setCachedCategories(categories: Category[]): void {
    try {
      ...
      if (!categories || categories.length === 0) {
        console.log(
          '⚠️ CategoriesService: No categories to cache, skipping cache save',
        );
        return;
      }
      ...
      console.log(
        '💾 CategoriesService: Categories cached for restaurant:',
        this.cacheService.getCurrentRestaurantId(),
        'Count:',
        categories.length,
