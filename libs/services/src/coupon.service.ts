...
import { OnlineOrderCoupon, CouponValidationResult, OrderType } from '@zitro/models';
...
export class CouponService {
   ...
   */
  getActiveCoupons(manualSearch: boolean = false): Observable<OnlineOrderCoupon[]> {
    // // Check cache first
          ...
          querySnapshot.forEach(doc => {
            ...
            if (couponData && typeof couponData === 'object') {
              const coupon: OnlineOrderCoupon = {
                ...
                isDisplayedForOnlineOrders: couponData['isDisplayedForOnlineOrders'] || false,
   ...
   */
  validateCoupon(couponCode: string, orderAmount: number, cartItems?: any[], orderType?: OrderType): Observable<CouponValidationResult> {
    return this.getCouponByCode(couponCode).pipe(
      switchMap(async coupon => {
        ...
        if (currentUserPhone && (coupon.maxUsagePerUser || coupon.cooldownPeriodDays)) {
          const userData = await this.userManagementService.getUserData(currentUserPhone);
          if (userData) {
            const usageHistory = userData.couponUsageHistory || [];
            const couponUsages = usageHistory.filter(usage => usage.couponCode === coupon.code);
            ...
            if (coupon.cooldownPeriodDays && couponUsages.length > 0) {
              ...
              if (daysSinceLastUse < coupon.cooldownPeriodDays) {
                const daysRemaining = coupon.cooldownPeriodDays - daysSinceLastUse;
                return {
                  isValid: false,
            ...
            if (coupon.maxUsagePerUser) {
              ...
              if (coupon.usagePeriod && coupon.usagePeriod !== 'lifetime') {
                const now = new Date();
                const periodStart = this.getPeriodStartDate(now, coupon.usagePeriod);
   ...
   */
  private filterActiveCoupons(coupons: OnlineOrderCoupon[], manualSearch: boolean): OnlineOrderCoupon[] {
    const today = new Date();
    today.setHours(0, 0, 0, 0); // Set to start of day
  ...
  private setCachedCoupons(coupons: OnlineOrderCoupon[]): void {
    try {
      ...
      console.log(`💾 Coupons cached successfully (${Math.round(duration / 1000 / 60 / 60)} hours)`);
    } catch (error) {
   ...
   */
  private getPeriodStartDate(currentDate: Date, period: 'day' | 'week' | 'month' | 'year' | 'lifetime'): Date {
   ...
   */
  private getPeriodText(period?: 'day' | 'week' | 'month' | 'year' | 'lifetime'): string {
    switch (period) {
      case 'day':
        return 'today';
      case 'week':
