...
export class RestaurantSwitchingService {
  ...
  private currentRestaurantSubject = new BehaviorSubject<Restaurant>(
    RESTAURANTS[0] as Restaurant,
  );
  ...
  private getCurrentRestaurantFromStorage(): Restaurant {
    ...
    if (selectedRestaurantId) {
      const restaurant =
        RestaurantSwitchingUtil.getRestaurantById(selectedRestaurantId);
      if (restaurant) {
        return restaurant;
      }
    }
  ...
  async switchRestaurant(
    ...
    try {
      ...
      const newRestaurant =
        RestaurantSwitchingUtil.getRestaurantById(restaurantId);
      ...
      if (currentRestaurant.id === restaurantId) {
        return {
          success: true,
          message: 'Already connected to this restaurant',
        };
      }
      ...
      console.log(
        '🔄 Starting restaurant switch from',
      ...
      const successMessage = RestaurantSwitchingUtil.getSwitchSuccessMessage(
        newRestaurant.name,
      );
      ...
      return {
        success: false,
        message: 'Failed to switch restaurant. Please try again.',
      };
    }
  }
