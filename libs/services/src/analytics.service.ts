...
export class AnalyticsService {
  ...
  async logAppVersion(
    appVersion: string,
    logAppVersionAnalytics: boolean,
    logAppVersionFirebase: boolean,
  ): Promise<void> {
    ...
    try {
      ...
      if (!alreadyLogged) {
        ...
        const userPhone =
          (await this.userManagementService.getCurrentUserPhone()) || 'guest';
        ...
        console.log(
          `📱 App version ${appVersion} logged for device ${deviceId} (${deviceType})`,
        );
      }
    } catch (error) {
  ...
  private async storeAppVersionUsage(data: {
    ...
    try {
      ...
      const userDocRef = doc(
        this.firestore,
        FIREBASE_COLLECTIONS.APP_VERSION_USES,
        documentId,
      );
      ...
      if (userDoc.exists()) {
        ...
        console.log(
          `✅ App version usage updated in Firebase for user ${data.userPhone} (${history.length + 1} records)`,
        );
      } else {
        ...
        console.log(
          `✅ App version usage created in Firebase for user ${data.userPhone} (first record)`,
        );
      }
    } catch (error) {
  ...
  private async storeSearchTerm(
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        console.log(
          `✅ Search term '${searchTerm}' stored in Firebase for user ${userPhone}`,
        );
      } else {
        ...
        console.log(
          `✅ Search term '${searchTerm}' created in Firebase for user ${userPhone} (first search)`,
        );
      }
    } catch (error) {
