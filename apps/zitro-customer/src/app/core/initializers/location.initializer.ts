  ...
  return async () => {
    try {
      ...
      if (selectedRestaurantId) {
        console.log('✅ Location Initializer: Restaurant already selected:', selectedRestaurantId);
        ...
        if (defaultBusinessId && defaultBusinessId === selectedRestaurantId) {
          console.log('✅ Location Initializer: Using default business, no location check needed');
          return;
        }
      }
      ...
      console.log('📍 Location Initializer: Preparing location services for business selection');
      
    } catch (error) {
      console.error('❌ Location Initializer: Error during initialization:', error);
