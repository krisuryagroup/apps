...
import { CreateOrderData, OrderItem as OrderItemModel, OrderType } from '@zitro/models';
...
import { VALIDATION_MESSAGES, FALLBACK_VALUES } from '../../core/constants/app.constants';
...
@Component({
  selector: 'app-cart',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, BottomNavComponent, DecimalPipe, OrderLoadingModalComponent, CouponSelectorComponent, CachedImageDirective, LoaderComponent, CallRestaurantButtonComponent],
...
export class CartComponent implements OnInit, OnDestroy {
  ...
  freeDeliveryThreshold: number = 250;
  ...
  // Packaging charges
  packagingChargesPerItem: number = 0;
  ...
  async ngOnInit() {
    ...
    try {
      const checkoutSettings = await this.orderService["appSettingsService"].getCheckoutSettings();
      this.packagingChargesPerItem = checkoutSettings.packagingChargesPerItem || 0;
      ...
      if (checkoutSettings.openTime && checkoutSettings.closeTime) {
        const now = new Date();
        const [openHour, openMinute] = checkoutSettings.openTime.split(":").map((v: string) => parseInt(v, 10));
        const [closeHour, closeMinute] = checkoutSettings.closeTime.split(":").map((v: string) => parseInt(v, 10));
  ...
  get isEligibleForFreeDelivery() {
    ...
    return this.pricingService.isEligibleForFreeDelivery(this.subtotal, this.pricingConfig);
  }
  ...
  get deliveryMessage() {
    ...
    return this.pricingService.getFreeDeliveryMessage(this.subtotal, this.pricingConfig);
  }
  ...
  async onOrderTypeChange(type: OrderType) {
    ...
    if (!this.orderConfigService.isOrderTypeEnabled(type)) {
      alert(`${this.orderConfigService.getOrderTypeDisplayName(type)} orders are currently not available`);
      return;
    }
  ...
  validateOrderType(): boolean {
    if (!this.selectedOrderType) {
      this.orderTypeError = this.orderConfigService.getMessage('general', 'orderTypeRequired');
    ...
    if (this.selectedOrderType === 'dine-in') {
      if (this.orderConfigService.shouldShowDineInDetails()) {
        if (!this.selectedTable || this.selectedTable.trim() === '') {
          this.orderTypeError = this.orderConfigService.getMessage('dineIn', 'tableRequired');
          setTimeout(() => {
        ...
        if (!table || !table.isAvailable) {
          this.orderTypeError = this.orderConfigService.getMessage('dineIn', 'tableUnavailable');
          setTimeout(() => {
            const element = document.querySelector('.dh-dinein-fields');
            if (element) {
              element.scrollIntoView({ behavior: 'smooth', block: 'center' });
    ...
    if (this.selectedOrderType === 'takeout') {
      if (this.orderConfigService.shouldShowTakeoutScheduledPickup()) {
        if (!this.scheduledPickupTime || this.scheduledPickupTime.trim() === '') {
          this.orderTypeError = this.orderConfigService.getMessage('takeout', 'pickupTimeRequired');
          setTimeout(() => {
    ...
    if (this.selectedOrderType === 'delivery') {
      if (!this.isAddressSelected) {
        this.orderTypeError = this.orderConfigService.getMessage('delivery', 'addressRequired');
        setTimeout(() => {
    ...
    if (this.appliedCoupon && this.selectedOrderType) {
      ...
      if (coupon.applicableOrderTypes && coupon.applicableOrderTypes.length > 0) {
        if (!coupon.applicableOrderTypes.includes(this.selectedOrderType)) {
          ...
          alert(`The coupon "${coupon.code}" is not applicable for ${this.selectedOrderType} orders.`);
        }
      }
    }
  }
  ...
  onTableSelect() {
    ...
    if (table && table.capacity) {
      ...
      if (this.numberOfGuests > table.capacity) {
        this.numberOfGuests = Math.min(this.orderConfig.dineInConfig.defaultGuests, table.capacity);
      }
    }
  }
  ...
  getPickupMessage(): string {
    return this.orderConfig?.messages?.takeout?.pickupMessage || 
    ...
    try {
      ...
      if (pricingSnap.exists()) {
        ...
        if (this.pricingConfig.delivery.enabled) {
          ...
          this.freeDeliveryThreshold = this.pricingConfig.delivery.free_delivery_above;
        }
      } else {
        console.warn('⚠️ Pricing configuration not found in Firebase. Using default values.');
        // Set default config
  ...
  get isAddressSelected() {
    ...
    return this.selectedAddressId !== '' || this.selectedLocationSnapshot.type !== 'none';
  }

  get selectedAddress() {
    return this.userAddresses.find(addr => this.getAddressId(addr) === this.selectedAddressId) || null;
  }
  ...
  getVariationLabel(item: any): string {
    if (item.hasVariations && item.selectedVariationId && item.variations) {
      const variation = item.variations.find((v: any) => v.id === item.selectedVariationId);
      ...
      if (phoneNumber) {
        ...
        if (this.userAddresses.length > 0) {
          ...
          if (matchedByLabel) {
            ...
            this.selectedAddressId = this.getAddressId(defaultAddress || this.userAddresses[0]);
          }
        }
      }
    } catch (error) {
  ...
  async placeOrder() {
    ...
    try {
      ...
      await this.orderProcessingService.processStageWithDelay('validating', 1500);
      ...
      const orderItems: OrderItemModel[] = this.cart
        ...
        .map(item => {
          ...
          if (item.hasVariations && item.selectedVariationId && item.variations) {
            const selectedVar = item.variations.find((v: any) => v.id === item.selectedVariationId);
            if (selectedVar) {
          ...
          const orderItem: OrderItemModel = {
            id: (item.id || item.name || FALLBACK_VALUES.UNKNOWN_ITEM).toString(),
      ...
      const deliveryChargeValue = this.isEligibleForFreeDelivery ? 0 : this.deliveryCharge;
      const totalValue = this.total || 0;

      // Create order data with no undefined values
      const orderData: CreateOrderData = {
        orderType: this.selectedOrderType!,
        tableNumber: this.selectedOrderType === 'dine-in' ? this.selectedTable : undefined,
        numberOfGuests: this.selectedOrderType === 'dine-in' ? this.numberOfGuests : undefined,
        scheduledPickupTime: this.selectedOrderType === 'takeout' && this.scheduledPickupTime ? new Date(this.scheduledPickupTime) : undefined,
        items: orderItems,
        subtotal: subtotalValue,
        deliveryCharge: deliveryChargeValue,
        total: totalValue,
        paymentMethod: (this.selectedPaymentMethod as 'cash' | 'online') || 'cash',
        deliveryAddress: this.selectedAddress && this.selectedOrderType === 'delivery' ? {
      ...
      await this.orderProcessingService.processStageWithDelay('processing', 1000);

      // Track payment and shipping info
      await this.analyticsService.logAddPaymentInfo(this.selectedPaymentMethod, this.total);
      ...
      await this.orderProcessingService.processStageWithDelay('confirming', 1500);
      ...
      // Show error state
      this.orderProcessingService.updateStage('error', 'Failed to place order. Please try again.', error instanceof Error ? error.message : 'Unknown error');
  ...
  private recalculateCouponOnCartChange(): void {
    ...
    this.couponService.validateCoupon(coupon.code, this.subtotal, this.cart, this.selectedOrderType || undefined).subscribe({
      next: (validationResult) => {
        if (validationResult.isValid) {
          ...
          console.log('Coupon recalculated successfully. New discount:', validationResult.discountAmount);
        } else {
          // Coupon is no longer valid, remove it
          this.appliedCoupon = null;
          console.log('Coupon removed due to cart changes:', validationResult.message);
          
          // Show a notification to user (you can customize this)
          this.showCouponRemovedNotification(validationResult.message || 'Coupon is no longer valid');
        }
      },
      error: (error) => {
