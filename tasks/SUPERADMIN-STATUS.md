...
| SA-002      | Compose shared admin screens (AD-002…AD-019) into this app's routes | M    | `[x]`  | done — all AD-002…AD-019 shared components wired into zitro-superadmin routes                                                                              |
| SA-003      | Feature Flags Management                                            | M    | `[x]`  | done — per-app/platform toggle list, upsert new flags; reads via GET /api/app-config, writes via PUT /api/admin/feature-flags/{app}                        |
| SA-004      | Translations Management                                             | L    | `[x]`  | done — lang selector, key/value table, search, add/edit/delete, CSV bulk import+preview, missing-key report vs EN_DEFAULTS                                 |
| SA-005      | Theme Management                                                    | M    | `[x]`  | done — list themes (built-in read-only), create/edit custom themes with token editor and live CSS-variable preview pane                                    |
| SA-006      | Per-App UI Config Management                                        | M    | `[x]`  | done — app selector + JSON editor (format, validate, save); embeds maintenance mode config in the same blob                                                |
