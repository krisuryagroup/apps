...
| RS-008                  | Menu Import — AI Photo/PDF Upload + Review & Approve      | XL   | `[ ]`  | ready — zitro-api TASK-038 done, but gated behind admin-toggled `ai_menu_import` flag (see zitro-api/TASK-STATUS.md — flag is seeded OFF, needs SuperAdmin to enable after ANTHROPIC_API_KEY is set). Frontend: parse response returns `{enabled:false}` when off — build the manual-add fallback for that case, don't treat it as an error. |
